...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', config('app.name', 'Laravel')); ?> - Blog</title>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center gap-8">
                    <a href="<?php echo e(route('blog.index')); ?>" class="text-xl font-bold serif-heading text-slate-900 dark:text-white hover:text-slate-700 dark:hover:text-slate-200 transition-colors duration-200">
                        Articulate
                    </a>
                    <div class="hidden md:flex gap-8">
                        <a href="<?php echo e(route('blog.index')); ?>" class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-medium transition-colors duration-200 relative group">
                            All Posts
                        ...
                        <?php if(auth()->guard()->check()): ?>
                            <a href="<?php echo e(route('user.profile', auth()->user()->id)); ?>" class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-medium transition-colors duration-200 relative group">
                                My Profile
                                <span class="absolute bottom-0 left-0 w-0 h-[2px] bg-slate-900 dark:bg-white group-hover:w-full transition-all duration-300"></span>
                            </a>
                        <?php endif; ?>
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    ...
                    <?php if(auth()->check()): ?>
                        <div class="hidden sm:flex items-center gap-3 pl-3 border-l border-slate-200 dark:border-slate-700">
                            <span class="text-slate-600 dark:text-slate-400 text-sm font-medium"><?php echo e(auth()->user()->name); ?></span>
                        </div>
                        <a href="<?php echo e(route('blog.create')); ?>" class="btn-primary text-sm px-4 py-2">
                            New Post
                        </a>
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="btn-secondary text-sm px-4 py-2">
                            Sign In
                        </a>
                    <?php endif; ?>
                </div>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        <li><a href="<?php echo e(route('blog.index')); ?>" class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white text-sm transition-colors duration-200">All Posts</a></li>
                        <?php if(auth()->guard()->check()): ?>
                            <li><a href="<?php echo e(route('blog.create')); ?>" class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white text-sm transition-colors duration-200">Write Post</a></li>
                            <li><a href="<?php echo e(route('user.profile', auth()->user()->id)); ?>" class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white text-sm transition-colors duration-200">My Profile</a></li>
                        <?php endif; ?>
                    </ul>
                </div>
                <div>
...
<?php /**PATH C:\Users\DELL\Desktop\PHP LARAVEL\mvc\resources\views/layouts/app.blade.php ENDPATH**/ ?>