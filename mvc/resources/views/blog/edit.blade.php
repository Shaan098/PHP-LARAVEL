...
<div class="container mx-auto px-4 py-12">
    <div class="max-w-3xl mx-auto animate-fade-in-up">
        <div class="mb-8">
            <a href="{{ route('blog.show', $post->slug) }}" class="inline-flex items-center gap-2 text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-semibold transition-colors mb-6">
                ← Back to Post
            </a>
            <h1 class="text-5xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent mb-2">
                ✏️ Edit Post
            </h1>
            <p class="text-gray-600 dark:text-gray-400 text-lg">Update your story</p>
        </div>

        <form action="{{ route('blog.update', $post) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-slate-900 p-8 rounded-xl shadow-lg dark:shadow-2xl space-y-6">
            @csrf
            @method('PUT')

            <!-- Title -->
            <div>
                <label for="title" class="block text-gray-900 dark:text-white font-semibold mb-3 text-lg">Post Title</label>
                <input 
                    type="text" 
                    id="title" 
                    name="title" 
                    class="w-full px-4 py-3 border-2 border-gray-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-900 transition-all duration-200 @error('title') border-red-500 @enderror" 
            ...
            <!-- Excerpt -->
            <div>
                <label for="excerpt" class="block text-gray-900 dark:text-white font-semibold mb-3 text-lg">Excerpt (Optional)</label>
                <textarea 
                    id="excerpt" 
                    name="excerpt" 
                    rows="2"
                    class="w-full px-4 py-3 border-2 border-gray-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-900 transition-all duration-200 @error('excerpt') border-red-500 @enderror" 
                    value="{{ old('excerpt', $post->excerpt) }}" 
                    placeholder="Brief summary that appears in listings..."
                    maxlength="500"
                ></textarea>
                <p class="text-gray-500 dark:text-gray-400 text-sm mt-2">Max 500 characters</p>
                @error('excerpt')
                    <p class="text-red-600 dark:text-red-400 text-sm mt-2 font-semibold">{{ $message }}</p>
                @enderror
            </div>
